...
import {
	type FullJavaStack,
	type FullWebAppStack,
	type IWebAppWizardContext,
} from "./IWebAppWizardContext";
import { getJavaLinuxRuntime } from "./stacks/getJavaLinuxRuntime";
import {
	type WebAppStackValue,
	type WindowsJavaContainerSettings,
} from "./stacks/models/WebAppStackModel";
...
export class WebAppCreateStep extends AzureWizardExecuteStep<IWebAppWizardContext> {
	...
	private async createWebApp(
		...
		return context.newSiteDomainNameLabelScope ===
			DomainNameLabelScope.Global
			? await this.createNewSite(context, rgName, siteName)
			: await this.createNewSiteWithDomainLabelScope(
					context,
					rgName,
					siteName,
				);
	...
	private async createNewSite(
		...
		return await client.webApps.beginCreateOrUpdateAndWait(
			rgName,
			siteName,
	...
	private async getNewSite(context: IWebAppWizardContext): Promise<Site> {
		const location = await LocationListStep.getLocation(
			context,
			webProvider,
		);
	...
	private async createNewSiteWithDomainLabelScope(
		context: IWebAppWizardContext,
		rgName: string,
		siteName: string,
	): Promise<Site> {
		...
		const authToken = (
			(await context.credentials.getToken()) as { token?: string }
		).token;
		const options: AzExtRequestPrepareOptions = {
			...
			body: JSON.stringify(
				await this.getNewSiteWithDomainLabelScope(context),
			),
		};
		...
		const sdkClient: WebSiteManagementClient =
			await createWebSiteClient(context);
		return await sdkClient.webApps.get(rgName, siteName);
	}

	private async getNewSiteWithDomainLabelScope(
		context: IWebAppWizardContext,
	): Promise<SitePayload> {
		const location = await LocationListStep.getLocation(
			context,
		...
		const sitePayload: SitePayload = {
			...
			properties: {
				autoGeneratedDomainNameLabelScope:
					context.newSiteDomainNameLabelScope,
				clientAffinityEnabled: true,
				serverFarmId: context.plan && context.plan.id,
				reserved: context.newSiteOS === WebsiteOS.linux, // The secret property - must be set to true to make it a Linux plan. Confirmed by the team who owns this API.
				siteConfig: newSiteConfig,
	...
	private addCustomLocationProperties(
		site: Site | SitePayload,
	...
	private getSiteConfig(context: IWebAppWizardContext): SiteConfig {
		...
		if (context.newSiteOS === WebsiteOS.linux) {
			newSiteConfig.linuxFxVersion =
				stack.stack.value === "java"
					? getJavaLinuxRuntime(
							...
							nonNullProp(context, "newSiteJavaStack")
								.minorVersion,
						)
					: nonNullProp(
			...
			const runtimeVersion: string = nonNullProp(
				stack.minorVersion.stackSettings,
