...
import {
	getCreatedWebAppMessage,
	showCreatedWebAppMessage,
} from "./showCreatedWebAppMessage";
...
export async function createWebApp(
	...
	node?: SubscriptionTreeItemBase | undefined,
	_nodes?: (SubscriptionTreeItemBase | undefined)[],
	suppressCreatedWebAppMessage: boolean = false,
): Promise<SiteTreeItem> {
	if (!node || !isSubscription(node)) {
		node = <SubscriptionTreeItemBase>(
			await ext.rgApi.tree.showTreeItemPicker(
	...
	const wizardContext: IWebAppWizardContext = Object.assign(
		context,
		node.subscription,
		{
	...
	try {
		...
		context.telemetry.properties.fileLoggingError = maskUserInfo(
...
export async function createWebAppAdvanced(
	context: IActionContext,
	node?: AzExtParentTreeItem | undefined,
): Promise<SiteTreeItem> {
