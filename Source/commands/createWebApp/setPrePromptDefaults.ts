...
export async function setPrePromptDefaults(
	wizardContext: IWebAppWizardContext &
		Partial<IDeployContext> &
	...
	const workspaceForRecommendation: WorkspaceFolder | undefined =
	...
	if (workspaceForRecommendation) {
		...
		if (await fse.pathExists(path.join(fsPath, "package.json"))) {
			...
			await fse.pathExists(path.join(fsPath, "requirements.txt"))
		) {
			wizardContext.recommendedSiteRuntime = ["python"];
		} else if (
			(
				await findFilesByFileExtension(
					workspaceForRecommendation.uri.fsPath,
					"csproj",
				)
			).length > 0
		) {
