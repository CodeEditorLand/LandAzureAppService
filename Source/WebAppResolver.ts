	...
	public async resolveResource(
		...
		return await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
						...
						uiUtils
							...
							.then((sites) => {
								for (const site of sites) {
									const siteName: string = nonNullProp(
										site,
										"name",
									);
									const count: number =
										(this.siteNameCounter.get(siteName) ??
											0) + 1;
				...
				const groupBy: string | undefined = getGlobalSetting<string>(
					"groupBy",
					"azureResourceGroups",
				);
				return new ResolvedWebAppResource(
					...
					nonNullValue(site),
					{
						...
						showLocationAsTreeItemDescription:
							groupBy === "resourceType" &&
							(this.siteNameCounter.get(
